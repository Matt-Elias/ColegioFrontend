...
const profesorService = {
  getDatosProfesor: async (token, idUsuario) => {
    try {
      console.log("[profesorService] Token recibido:", token);
      console.log("[profesorService] ID Usuario recibido:", idUsuario);
      
      if (!token || !idUsuario) {
        throw new Error('Token o ID de usuario no proporcionado');
      }

      const response = await axios.get(`${API_URL}/usuario/buscarProfesor/${idUsuario}`, {
        headers: { 
      ...
      console.log("[profesorService] Respuesta completa:", response);
